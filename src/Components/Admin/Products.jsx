...
const Products = () => {
  ...
  const handleAddOrUpdateProduct = async () => {
    ...
    formData.append('classYear', form.classYear);
  ...
  const handleDeleteSelected = async () => {
    const idsToDelete = selectedIndexes.map((i) => products[i]._id);

    try {
      await Promise.all(idsToDelete.map((id) => axios.delete(`http://localhost:4000/api/products/${id}`)));
      fetchProducts();
      setSelectedIndexes([]);
    } catch (err) {
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={6} md={2}>
            <FormControl fullWidth>
              ...
              >
                {(yearOptionsByDegree[form.degree] || []).map((year) => (
          ...
          <Grid item xs={6} md={2}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                disabled={!form.degree}
              >
                {(streamOptionsByDegree[form.degree] || []).map((stream) => (
